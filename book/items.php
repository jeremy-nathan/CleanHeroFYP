	...
	<div id="wrapper" class="clearfix">
		...
		<section id="page-title" class="bg-color page-title-dark py-6">
			...
			<div class="container clearfix">
				<h1>BOOKING FORM</h1>
				<span >Please fill in the form below to select the items</span>
				<ol class="breadcrumb">
				...
				<div class="container mb-12">
					<div class="row justify-content-between">
						<div class="col-lg-12 mb-12 mb-lg-12">
							<h3>Choose the items available</h3>
							<h3 id="data"></h3>
							<div class="form-widget">

								<div class="form-result"></div>

								<form class="mb-0" id="template-contactform" name="template-contactform" action="./quotation.php" method="post">
									...
									<div class="row">
										...
										<div class="col-12 form-group">
											...
											<?php
											// Output the pricing for each Item 
												foreach($_SESSION["priceList"] as $sub){
													$size=$sub["Size"];
													$price=$sub["Price"];
													echo '<label for="item[]">'.$sub["Item"].'</label>
														  <select required name="item[]" class="form-control required">
																<option value="">-- Select One --</option>';
													for($i=0;$i<count($size);$i++){
														echo '<option value="'. $size[$i] .' '. $price[$i] .' '. $sub["Item"] .'">'. $size[$i] .' : '. $price[$i] .'</option>';
													}
													echo '</select><br>';
												}
											?>												
