...
class Firestore{
    ...
    public function __construct(string $collection){
        $this->db = new FirestoreClient([
            // 'keyFilePath' => 'D:/Downloads/fyp-cms-517aa7d5b2b5.json'
            'projectId' => 'fyp-cms'
        ]);
    ...
    public function getID(string $field, string $operator,string $value){
        $id="";
        ...
        $id=[];
        $query = $this->db->collection($this->name)->documents();
        foreach ($query as $value) {
                if($value->exists()){
                    $id[]=$value->id();
                }
    ...
    public function updateDocument(string $docName,array $newData){
        ...
        public function createSubCollection(string $name,string $doc_name,array $data=[]){
            try {
                $this->db->collection($this->name)->document($doc_name)->collection($name)->add($data);
                return true;
            } catch (Exception $exception) {
                return $exception->getMessage();
            }
